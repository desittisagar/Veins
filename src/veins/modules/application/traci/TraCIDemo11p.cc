...
//Coord rsu_pos=Coord(2000.0,2000.0,3.0);

//vector<struct neighbour> v;
//vector<WaveShortMessage*> m;
  ...
  /*  if(wsm->getRecipientAddress() == myId)
    {
          ...
          wsm->setSenderAddress(myId);
          wsm->setSerial(3);
          scheduleAt(simTime() + 2 + uniform(0.10,0.2), wsm->dup());
    }
    else{
        std::cout<<"did not get it"<<endl;
...
void TraCIDemo11p::handlePositionUpdate(cObject* obj) {
    ...
    //cout<<"Min_Dist "<<*min_element(distance_,distance_+j)<<endl;
    //for()
    //sendDown(bsm);
}
